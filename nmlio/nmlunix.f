...
********Leading dimension declarations for namelist input (only!) *********
     ...
     &          maxny,maxs0
      parameter (maxnstat =  300)           ! max n of equations (& responses)
      parameter (maxnobs  =  2000)          ! max n of observations/data set
      parameter (maxnsets =  100)           ! max n of datasets

      integer*4 maxngpar,maxnlpar
      parameter (maxngpar =  1000)           ! max n of global parameters
      parameter (maxnlpar =  1000)           ! max n of local parameters

      integer*4 maxnx,maxnopt,maxnopt2,maxnopt4,maxnnew
      parameter (maxnx     = 1000)          ! max n of independent variables
      parameter (maxnopt   = 1000)           ! max n of parameters in
                                            ! optimization, simulation, sensitivity
      parameter (maxnopt2  = 2*maxnopt,maxnopt4 = 4*maxnopt)
      parameter (maxndir   = maxnopt2)
      parameter (maxnnew   = maxnopt*maxnsets)
      parameter (maxs0     = maxnsets*maxnstat)
      parameter (maxny     = maxnsets*maxnobs)

***************SMBL variables **********
      integer*4  mxmsent,mxwords ,mxchars
c      parameter(mxmsent = 2000,    !max numb. of modelvar-sent.
c     *          mxwords = 6000,    !max numb. of words in input-strs
c     *          mxchars = 400000)  !max length of strs modelvar,...
      parameter(mxmsent = 4000,    !max numb. of modelvar-sent.
     *          mxwords = 12000,    !max numb. of words in input-strs
     *          mxchars = 800000)  !max length of strs modelvar,...
c       mxchars = max(len(modelvar,len(target),len(estvar))
       integer*4 error_number,ngrid

      integer*4 ntf(maxnsets)
cnoall      integer*4 ntf[ALLOCATABLE](:)
...
************** LOCAL PARAMETERS ***********************************
      ...
      character*20000 modelvar !! oli 8000 ja 1000
      character*10000 target,estvar          ! symbolic names in nml
...
cnoall       integer*4    newobs[ALLOCATABLE](:,:)
       integer*4    newobs(maxnnew,2)
*     Model parameters
       integer*4    ngpar,nlpar,nstates,nx,nsaux
       real*8       gpar(maxngpar)
cnoall       real*8       lpar[ALLOCATABLE](:,:)
       real*8       lpar(maxnlpar,maxnsets)
       integer*4    ifile,nstates0
cnoall       integer*4    s0file[ALLOCATABLE](:,:)
       integer*4    s0file(maxnstat,maxnsets)

*     Parameters of data
       integer*4    nsets,nsaveset

*     Data set parameters
cnoall       real*8       rnobs[ALLOCATABLE](:)  !******
cnoall       integer*4    nobs[ALLOCATABLE](:),ncolxy[ALLOCATABLE](:)
cnoall       integer*4    indx[ALLOCATABLE](:,:)
cnoall       integer*4    indy[ALLOCATABLE](:,:)
cnoall       integer*4    nydata[ALLOCATABLE](:),nnydata[ALLOCATABLE](:,:)
cnoall       real*8       t0[ALLOCATABLE](:),tf[ALLOCATABLE](:,:),
cnoall     &              states0[ALLOCATABLE](:,:)

       real*8       rnobs(maxnsets)  !******
       integer*4    nobs(maxnsets),ncolxy(maxnsets)
       integer*4    indx(maxnx,maxnsets)
       integer*4    indy(maxnstat,maxnsets)
       integer*4    nydata(maxnsets),nnydata(maxnobs,maxnsets)
       real*8       t0(maxnsets),tf(maxnobs,maxnsets),
     &              states0(maxnstat,maxnsets)
...
cnoall       real*8       smin[ALLOCATABLE](:),smax[ALLOCATABLE](:)
       real*8       smin(maxnstat),smax(maxnstat)
...
c      data igrid     /maxnopt2*0/
      data dstep     /1.0d-04/
       data smin   /maxnstat*0/
       data smax   /maxnstat*0/
...
*     The parameters of the data
     ...
     &                  nobs, ncolxy, indy, indx, nydata !,t0, tf
      ...
      data obss0     /0/
      data nsaveset  /1/
       data nobs       /maxnsets*0/
       data ncolxy     /maxnsets*0/
       data t0         /maxnsets*0/
...
c
c write the name of the nmlfile to file for use in MCMC run
c 2007-01-09
      open(unit=1,file='mcmcnml.txt',status='replace')
      write(1,*) nmlfile
      close(1)
      open(unit=1,file = nmlfile, status = 'old')      
...
cnoall      maxnsets = max(nsets,100) !just to allow more data set definitions
cnoall      maxnstat = nstatea

cnoall      allocate (ntf(maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for ntf '

cnoall      allocate (lpar(maxnlpar,maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for locals'

cnoall      allocate (s0file(nstatea,maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for s0 '

cnoall      allocate (rnobs(maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for nobs'

cnoall      allocate (nobs(maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for nobs'

cnoall      allocate (ncolxy(maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for ncolxy'

cnoall      allocate (indx(maxnx,maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for indx'

cnoall      allocate (indy(maxnstat,maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for indy'

cnoall      allocate (nydata(maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for nydata'

cnoall      allocate (t0(maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for t0'

cnoall      allocate (states0(maxnstat,maxnsets),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for states0'

cnoall      allocate (smin(maxnstat),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for smin'

cnoall      allocate (smax(maxnstat),stat=error)
cnoall      if (error.ne.0) stop 'Not enough storage for smax'

cnoall      do k = 1,maxnsets
cnoall         nobs(k)   = unini
cnoall         nydata(k) = unini
cnoall         ncolxy(k) = unini
cnoall         t0(k)     = 0.0d0
cnoall         do i = 1,maxnx
cnoall           indx(i,k) = unini
cnoall         enddo
cnoall         do i = 1,maxnstat
cnoall           indy(i,k)    = unini
cnoall           s0file(i,k)  = 0
cnoall           states0(i,k) = uninit
cnoall         enddo
cnoall      enddo
cnoall      do i = 1,maxnstat
cnoall         smin(i) = 0.0d0
...
cnoall      if (nsets.gt.1) then
cnoall       do k = 2,nsets
cnoall         if (nobs(k).eq.unini)   nobs(k)   = nobs(1)
cnoall         if (nydata(k).eq.unini) nydata(k) = nydata(1)
cnoall       enddo
cnoall      endif

cnoall       do k=1,nsets
cnoall         maxnobs = max(maxnobs,nobs(k))
cnoall       enddo
cnoall       maxnobs   = maxnobs+1
cnoall       maxny     = maxnobs*maxnsets
cnoall       maxnnew   = maxny
cnoall       allocate (nnydata(maxnobs,maxnsets),stat=error)
cnoall       if (error.ne.0) stop 'Not enough storage for nnydata'
cnoall
cnoall       allocate (tf(maxnobs,maxnsets),stat=error)
cnoall       if (error.ne.0) stop 'Not enough storage for tf'
cnoall
cnoall       allocate (newobs(maxnnew,2),stat=error)
cnoall       if (error.ne.0) stop 'Not enough storage for newobs '
...
****** the objective function ****************************************
      ...
      elseif (task(1:3).eq.'exp' .and. objunini) then
         objfun(1:3) = 'exp'
      elseif (task(1:3).eq.'exp' .and. optcrit(1:4).eq.'glsq'
     &                           .and. objunini) then
         objfun(1:3) = 'exp'
...
c      integer*2     argstat
c      ftype = 'for'
c      call getarg(argno, argbuf, argstat)
